...
//    val outputDir = "D:\\tmp"
    ...
    val csvInput = File("/mnt/evo860/home/stefanwuersten/thesis/statistics/csv-rxjava")
    ...
    csvInput.walk().forEach {
        if (it.isFile) {
            try {
                ...
                val items5 = getItems(all, 5, reachedIterationDivergence.getValue(name))
                val items10 = getItems(all, 10, reachedIterationDivergence.getValue(name))
                val items15 = getItems(all, 15, reachedIterationDivergence.getValue(name))
                val items20 = getItems(all, 20, reachedIterationDivergence.getValue(name))
                val items25 = getItems(all, 25, reachedIterationDivergence.getValue(name))
                val items50 = getItems(all, 50, reachedIterationDivergence.getValue(name))
...
private fun getItems(items: Collection<HistogramItem>, iter: Int, iterations: Map<Int, Int>): List<HistogramItem> {
    ...
    iterations.forEach { f, reached ->
        ...
        val filter = items.filter { it.fork == f && it.iteration > reachedFixed && it.iteration <= reachedFixed + iter }
        all.addAll(filter)
    }
    return all
}

private fun parseInput(input: File, reachedIteration: MutableMap<String, MutableMap<Int, Int>>) {
    input.forEachLine {
